    ...
    public function index()
    {
        ...
        if ($this->request->getMethod() === 'post' && $this->validate(
            ...
            if (! empty($_FILES['gambar']['name'])) {
                ...
                $avatar->move(WRITEPATH . '../assets/upload/undangan/', $namabaru);
                // Create thumb
                $image = \Config\Services::image()
                    ->withFile(WRITEPATH . '../assets/upload/undangan/' . $namabaru)
                    ->fit(100, 100, 'center')
                    ->save(WRITEPATH . '../assets/upload/undangan/thumbs/' . $namabaru);
                // masuk database
                // masuk database
                ...
                return redirect()->to(base_url('admin/undangan'));
            }
            // masuk database
            $data = ['id_user'      => $this->session->get('id_user'),
                'id_kategori_staff' => $this->request->getPost('id_kategori_staff'),
        ...
        $data = ['title'     => 'Data Staff: ',
            ...
            'satu' => $m_staff->kategori(1),
            'dua' => $m_staff->kategori(2),
            'tiga' => $m_staff->kategori(3),
            'empat' => $m_staff->kategori(4),
            'lima' => $m_staff->kategori(5),
            'content'        => 'admin/undangan/index',
        ];
